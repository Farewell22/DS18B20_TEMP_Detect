...
float slide_windows_get(float value)
{
    float output = 0;
    if(temps.buffer_cur < SLIDING_WINDOW)
    {
        temps.window_buffer[temps.window_cur++] = value; //先把窗口填满
        output = value;
    }else{
        float sum = 0;
        memcpy(&temps.window_buffer[0],&temps.window_buffer[1],(SLIDING_WINDOW - 1)*sizeof(float));
        temps.window_buffer[SLIDING_WINDOW - 1] = value;
        for(uint8_t i = 0;i<SLIDING_WINDOW)
    }
    return output;
...
{
   temps.buffer[temps.buffer_cur++] = temp;
   if(temps.buffer_cur>=BUFFER_SIZE)
   {
    get_buffer();
    temps.buffer_cur = 0;
