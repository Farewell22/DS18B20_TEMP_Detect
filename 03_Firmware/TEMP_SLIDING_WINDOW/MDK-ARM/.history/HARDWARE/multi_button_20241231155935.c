...
void key1_pressdown_callback(void *btn)
{
	printf("SINGLE:CLICK\r\n");
}

void key1_doubleclick_callback(void *btn)
{
	printf("DOUBLE:CLICK\r\n");
}

void key1_longpress_callback(void* btn)
{
...
void key_init(void)
{
	...
	//  button_attach(&key1,LONG_PRESS_HOLD,key1_longpress_callback);
	 button_attach(&key1,LONG_PRESS_START,key1_longpress_callback);
	 button_start(&key1);
	 multiTimerStart(&key_timer,5,Timerkeycallback,NULL);
	 uint8_t state = (uint8_t)HAL_GPIO_ReadPin(KEY_GPIO_Port,KEY_Pin);
	 printf("%d\r\n",state);
}
